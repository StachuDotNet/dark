...
One consequence worth knowing: an isolated store usually looks like a FIRST RUN, so the workbench greets
with "Welcome to Darklang" rather than "Welcome back", and a test waiting for the returning-user greeting
waits forever. The greeting reads commits, uncommitted changes and whether a relay is set, so clearing the
config contributes (no relay) but a store with a commit in it still greets you back.
