...
let private cliExe : string =
  ...
  |> List.tryPick (fun dir ->
    if System.IO.Directory.Exists dir then
      System.IO.Directory.EnumerateFiles(dir, "Cli", System.IO.SearchOption.AllDirectories)
      |> Seq.tryHead
    else
      None)
...
// ── process helpers ─────────────────────────────────────────────────────────────────────────────────
let private startInfo (dir : string) (args : string list) : System.Diagnostics.ProcessStartInfo =
  let psi = System.Diagnostics.ProcessStartInfo()
  psi.FileName <- cliExe
...
let private mk (root : string) (name : string) : Instance =
  ...
  System.IO.Directory.CreateDirectory(System.IO.Path.Combine(dir, "logs")) |> ignore<System.IO.DirectoryInfo>
  ...
  System.IO.Directory.CreateDirectory(System.IO.Path.Combine(dir, "rundir")) |> ignore<System.IO.DirectoryInfo>
  System.IO.File.Copy(seedDb.Force(), System.IO.Path.Combine(dir, "data.db"), overwrite = true)
  { name = name; dir = dir }

let private dbOf (inst : Instance) : string = System.IO.Path.Combine(inst.dir, "data.db")
    ...
    if not healthy then
      ...
      let killed =
        ...
        with _ ->
          false
      ignore<bool> killed
      Exception.raiseInternal $"serve {inst.name} never became healthy on port {port}" []
    return p
  }
...
let private assertRealHash (label : string) (h : string) : unit =
  Expect.isTrue (h.Length >= 8) $"{label}: expected a real (non-empty) hash line, got '{h}'"
  let low = h.ToLower()
  Expect.isFalse
...
let private withRoot (name : string) (body : string -> Task<unit>) : Task<unit> =
  task {
    let root = System.IO.Path.Combine("/tmp", $"dark-e2e-{name}-{System.Guid.NewGuid()}")
...
let private realTests =
  ...
  <| testList
    "SyncE2E"
    [ testTask "basic sync: a committed change pulls over real HTTP and converges (hash + value)" {
        do!
      ...
      testTask "conflict race: concurrent binds record a conflict; keep-mine propagates and converges" {
        do!
          withRoot "conflict" (fun root ->
            task {
              ...
              try
                ...
                darkIn a.dir [ "conflicts"; "keep-mine"; "SyncTest.Race.n" ] |> ignore<string>
                darkIn b.dir [ "sync" ] |> ignore<string>
                assertRealHash "A's post-resolution hash" (hashOf a "SyncTest.Race.n")
                Expect.equal
      ...
      testTask "conflict race: keep-theirs adopts the PEER's value (overriding the local LWW winner) + converges" {
        do!
          withRoot "conflictT" (fun root ->
            task {
              ...
              try
                ...
                |> ignore<string>
                darkIn b.dir [ "sync" ] |> ignore<string>
                assertRealHash "A's post-keep-theirs hash" (hashOf a "SyncTest.RaceT.n")
                Expect.equal
      ...
      testTask "branch sync: a branch created on A appears on B over real HTTP (exercises the branch-op wire)" {
      ...
      testTask "offline peer: syncing against an unreachable peer fails honestly + leaves the store intact" {
        ...
        do!
          withRoot "offline" (fun root ->
            task {
              ...
              Expect.isTrue
                (out.Contains "reach" || out.Contains "couldn't" || out.Contains "unreachable")
              ...
              Expect.isFalse (ver = "") "the CLI + store are still usable after the failed sync"
            })
      } ]
