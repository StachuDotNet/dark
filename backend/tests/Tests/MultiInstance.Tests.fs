...
let private pin
  ...
  (decidedAt : string)
  : PT.PackageOp =
  PT.PackageOp.Decide("propagation", loc name, "pin", reason, decidedAt)
...
///
/// This is the claim that separates this sync from one that ships source. A pin is a `Decide` op, so it
