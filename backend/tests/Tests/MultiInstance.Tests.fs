...
let private deprecateEvent
  ...
  let op = PT.PackageOp.Deprecate(target, PT.DeprecationKind.Obsolete, "obsolete (test)")
  let opId = Inserts.computeOpHash op
  (opId, BS.PT.PackageOp.serialize opId op, PT.mainBranchId, commitHash, ts)
...
let tests =
  ...
  <| testList
    ...
    [ testTask "fresh instances are isolated stores, and branch structure syncs across the wire" {
      ...
      testTask "branch identity is GLOBAL: a synced branch keeps the SAME id + name (never re-minted / instance-scoped)" {
      ...
      testTask "deprecation converges: a Deprecate authored on A folds into `deprecations` on B via the wire" {
        ...
        try
          ...
          let deprecatedCount () : Task<int64> =
            ...
            |> Sql.executeRowAsync (fun read -> read.int64 "n")
          let! _ = Seed.receiveOps [] [ deprecateEvent fnHash commit "2026-07-08T12:00:00.000Z" ]
          ...
          Expect.isGreaterThan aDep 0L "A folded the Deprecate op into the deprecations projection"
          Expect.equal bBefore 0L "B didn't have the deprecation before sync (isolated seed)"
          Expect.isGreaterThan bAfter 0L "B converged: the deprecation arrived + folded via the wire"
        finally
          teardown insts
      }
      ...
      testTask "cross-store conflict: concurrent binds to one name converge (LWW) + record a conflict" {
        ...
        try
          ...
          Expect.equal bConflicts 1L "B recorded exactly this divergence as a conflict (never silent)"
        finally
          teardown insts
      }
      ...
      testTask "resolution survives a projection rebuild — rebuildProjections re-applies the overlay" {
        ...
        try
          ...
          Expect.equal beforeRebuild [ fnA ] "the override took (fnA, not the LWW winner fnB)"
      ...
      testTask "dark conflicts renders a real recorded conflict (regression: short-hash slice was Int32)" {
      ...
      testTask "dark conflicts on a converged store reports no conflicts (the clean path)" {
        let mutable insts : List<Instance> = []
        try
          let! a = seededInstance "a"
          insts <- [ a ]
      ...
      testTask "the in-process CLI driver reads the active store (dispatch + rendering work end to end)" {
      ...
      testTask "an instance can view a real seeded package item — `dark view Stdlib.Option.Option`" {
        ...
        try
          ...
          Expect.stringContains out "Some" "…and shows its cases (Some/None) — a genuine view, not an error"
          let low = out.ToLower()
          Expect.isFalse
