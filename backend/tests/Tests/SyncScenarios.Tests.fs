...
let private recv (event : System.Guid * byte[] * System.Guid * string * string) : Task<int64> =
...
let private wipe (loc : PT.PackageLocation) (opIds : List<System.Guid>) : Task<unit> =
  task {
  ...
  <| testList
    ...
    [ // CONVERGENCE — the core guarantee: concurrent edits to the SAME name settle on the same winner no
      ...
      testTask "convergence: concurrent SetName to one name is order-independent (LWW)" {
        ...
        Expect.equal r2 [ hB ] "new→old ALSO converges to the newer hash (late older op skipped)"
      ...
      testTask "idempotency: re-receiving the same op folds 0 and leaves the binding unchanged" {
        let! (hA, _) = twoFns ()
        let! commitHash = aCommit ()
        let loc : PT.PackageLocation =
      ...
      testTask "conflict → resolution: keep-mine and keep-theirs each override the LWW winner" {
        ...
        Expect.equal kept [ hLocal ] "keep-mine: the resolution overrides LWW back to the local hash"
