...
let convergenceOrderIndependent =
  // CONVERGENCE — the core guarantee: concurrent edits to the SAME name settle on the same winner no matter
  // which instance's op arrives first. (origin_ts LWW: newer wins; older is skipped on replay.)
  testTask "convergence: concurrent SetName to one name is order-independent (LWW)" {
    let! (hA, hB) = twoFns ()
    ...
    Expect.equal r1 r2 "order-independent — the two-instance convergence property"
...
let idempotentReReceive =
  // IDEMPOTENCY — re-receiving the same op set changes nothing and folds 0 (so "Pulled N" stays honest and a
  // re-pull can't corrupt the binding).
  testTask
...
let conflictResolutionOverridesLww =
  // CONFLICT → RESOLUTION → CONVERGE, both directions. A divergence auto-resolves by LWW but is recorded; a
  // human override (keep-mine / keep-theirs) mints a Resolution whose overlay wins and converges.
  testTask
...
let eventsSincePaginates =
  // PAGINATION / CURSOR RESUME — the serve read (eventsSince) returns one bounded batch and a resume cursor;
  // the next read after that cursor is the next batch, disjoint (so a puller loops to catch up and an
...
let branchSyncStructureContentMerge =
...
let tests =
  ...
  <| testList
    ...
    [ convergenceOrderIndependent
      idempotentReReceive
      conflictResolutionOverridesLww
      noPhantomConflictOnRepull
      eventsSincePaginates
      branchSyncStructureContentMerge ]
