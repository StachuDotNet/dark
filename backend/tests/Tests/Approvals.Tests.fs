...
let testAddItemToOwnNamespaceBranch =
  testTask "add item to own namespace (branch) - starts pending" {
    ...
    // Create a branch
    let! branchResult = Branches.create ownerId "test-branch"
    let (branch : Branches.Branch) = Result.unwrap branchResult
...
let testAddItemToOthersNamespaceBranch =
  testTask "add item to others' namespace (branch) - pending" {
    ...
    let! branchResult = Branches.create contributorId "contrib-branch"
    let (branch : Branches.Branch) = Result.unwrap branchResult
