...
let tests =
  ...
  <| testList
    ...
    [ testTask
      ...
      testTask
        "receiveOps is idempotent and reports zero newly-applied for known ops" {
        let! events =
          Sql.query
        ...
        Expect.isTrue
          ...
          "there are committed ops to re-receive"
      ...
      testTask "receiveOps appends a new op preserving origin_ts and folds it" {
        ...
        let! _ =
          Seed.receiveOps
            []
            [ (opId, opBlob, PT.mainBranchId, commitHash, distinctiveTs) ]
      ...
      testTask
        "conflict → resolution → converge: divergence detected, LWW applied, human override wins" {
        ...
        let cleanup () : Task<unit> =
          task {
            ...
            do!
              Sql.query
                "DELETE FROM sync_conflicts WHERE location = 'Test.Diverge.dv'"
              |> Sql.executeStatementAsync
            do!
        ...
        let! _ =
          Seed.receiveOps
            []
            [ (idA, blobA, PT.mainBranchId, commitHash, "2099-01-01T00:00:00.001Z") ]
        // #2: a peer's op rebinds it to B — that's a divergence (curHash=A ≠ incoming=B), auto-resolved by LWW.
        let! _ =
          Seed.receiveOps
            []
            [ (idB, blobB, PT.mainBranchId, commitHash, "2099-01-01T00:00:00.002Z") ]
      ...
      testTask
        "a re-received op reconciles origin_ts to the MIN in package_ops + locations (convergence)" {
        let! fnHash =
          Sql.query "SELECT hash FROM package_functions LIMIT 1"
          |> Sql.executeRowAsync (fun read -> read.string "hash")
        let! commitHash =
        ...
        let recv (ts : string) : Task<unit> =
          task {
            let! _ =
              Seed.receiveOps [] [ (opId, opBlob, PT.mainBranchId, commitHash, ts) ]
            ()
          }
        ...
        Expect.equal
          loc2
          [ "2099-01-01T00:00:00.050Z" ]
          "binding origin_ts refreshed to the MIN (050)"
        ...
        Expect.equal
          ts3
          "2099-01-01T00:00:00.050Z"
          "a later stamp never raises the reconciled MIN"
