...
let tests =
  testSequenced
  <| testList "OpsProjections" [
    testTask "rebuildProjections deterministically regenerates projections from the op log" {
      ...
      Expect.equal fns2 fns1 "package_functions: a re-rebuild reproduces the same projection"
      ...
      Expect.equal blobsAfter blobsBefore "package_blobs (canonical content) preserved, not dropped"
    }

    // the projection registry — the "small new abstraction" (fold/dirty descriptors)
    ...
    test "projectionsDirtiedByBatch unions a batch's dirtied projections; AddFn-only skips locations" {
      ...
      let mixed = Seed.projectionsDirtiedByBatch (Set.ofList [ "AddFn"; "SetName" ])
      Expect.equal
        mixed
    ...
    testTask "rebuildDirtied {AddFn} refolds functions but leaves locations untouched (selective)" {
      ...
      Expect.isTrue (fnsAfter > 0L) "package_functions regenerated from the AddFn ops"
