/// Tests for the conflict-dispatch skeleton (LibExecution.RuntimeTypes
/// Conflict/Resolution/ConflictDispatch + ExecutionState.conflictDispatch).
/// Verifies the default dispatch is FailLoudly (byte-identical to today) and
...
let tests =
  testList "ConflictDispatch" [
    testTask "default dispatch FailLoudly-s a RuntimeError unchanged" {
      let state = freshState ()
      let err = RTE.FnNotFound aName
      let! res = state.conflictDispatch (RT.CRuntimeError err) (ctx state) |> Ply.toTask
      match res with
      | RT.RFailLoudly e -> Expect.equal e err "FailLoudly carries the same RuntimeError"
      | _ -> failtest "default dispatch should FailLoudly"
    }

    testTask "default dispatch maps FnNotFound to a FailLoudly FnNotFound" {
      let state = freshState ()
      let! res = state.conflictDispatch (RT.CFnNotFound aName) (ctx state) |> Ply.toTask
      match res with
    ...
    testTask "an installed policy overrides the default (Substitute)" {
      ...
      let state =
        { baseState with
            conflictDispatch = fun _ _ -> uply { return RT.RSubstitute(RT.DInt64 0L) } }
      let! res = state.conflictDispatch (RT.CFnNotFound aName) (ctx state) |> Ply.toTask
      match res with
      | RT.RSubstitute(RT.DInt64 n) -> Expect.equal n 0L "installed policy substituted"
      | _ -> failtest "installed policy should Substitute, proving the hook is swappable"
    }

    // CSyncDivergence — two peers bound the same name to different content (sync.md). It flows
    // through the SAME conflict policy: strict default fails loudly; a sync policy resolves.
    // (Mirrors the integrated `compose-check` test so the leaf stays consistent with it.)
    testTask "a sync divergence fails loudly by default and is policy-resolvable (last-writer)" {
      let state = freshState ()
      ...
      | RT.RFailLoudly(RTE.UncaughtException(msg, _)) ->
        Expect.stringContains msg "sync divergence" "default surfaces the divergence loudly"
        Expect.stringContains msg "Stachu.foo" "the diverged location is named"
      | _ -> failtest "default dispatch should FailLoudly (surface) a sync divergence"
        ...
        { state with
            conflictDispatch =
              fun conflict _ ->
                uply {
                  match conflict with
                  | RT.CSyncDivergence(_, _, incoming) -> return RT.RSubstitute(RT.DString incoming)
                  | _ -> return RT.RFailLoudly(RTE.UncaughtException("unexpected", []))
                } }
