...
///
/// `applied = 1` means "this op is in the projections". Setting it on an op nothing could read would be
/// false, and it is the difference between a downgrade being awkward and being lossy: an older binary
/// meeting newer ops would mark them applied, and the newer binary would then never fold them again.
/// Leaving them unapplied costs a re-read per fold and keeps them recoverable.
let foldQuarantinesPoison =
  testTask
    ...
    Expect.equal
      applied
      0L
      "left unapplied, so a build that CAN read it still folds it"
    do! cleanup ()
