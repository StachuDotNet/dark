/// Tests the sync wire's core safety property (pr-sync-read-write): applying the
    ...
    [ testTask
      ...
      testTask
        ...
        try
          ...
          // store B's canonical op-log table (FK to branches dropped — standalone op stream)
          exec
      ...
      // bootstrap: a snapshot is the whole log + a consistent cursor (derived from the same read)
      testTask
        "Sync.snapshot returns the whole log with a consistent bootstrap cursor" {
        let! (ops, cursor) = Sync.snapshot ()
        let! total = countOps ()
        Expect.equal
          (int64 (List.length ops))
          total
          "the snapshot contains the whole op log"

        // the cursor is the snapshot's own max rowid — consistent by construction
        let maxInSnapshot = ops |> List.map (fun (r, _, _, _) -> r) |> List.max
        Expect.equal
          cursor
          maxInSnapshot
          "the bootstrap cursor is the snapshot's max rowid"

        // a new peer polling from the snapshot cursor sees no already-included ops
        let! afterSnapshot = Inserts.opsSince cursor
        Expect.isEmpty
          afterSnapshot
          "polling from the snapshot cursor returns nothing already in the snapshot"
      }
      ...
      // Sync.reconcileDivergence — the op-emitting "fix" half of divergence handling (the
      // ConflictDispatch policy decides the strategy; this builds the reconciling op). Pure: a
      // divergent incoming SetName + a strategy → the op (or none). No DB.
      test "Sync.reconcileDivergence emits the right reconciling op per strategy" {
        let location : PT.PackageLocation =
          { owner = "Stachu"; modules = [ "Foo" ]; name = "bar" }
        let kind = PT.ItemKind.Fn
        let existingHash = "hashA"
        let incomingHash = "hashB"
        let incoming =
          PT.PackageOp.SetName(
            location,
            PT.Reference.fromHashAndKind (PT.Hash incomingHash, kind)
          )

        // LastWriterWins: the incoming SetName already applied = the winner → no reconciling op
        Expect.isNone
          (Sync.reconcileDivergence Sync.LastWriterWins incoming existingHash)
          "last-writer-wins needs no op — the applied incoming bind IS the resolution"

        // Fail: leave the divergence surfaced → no auto-op
        Expect.isNone
          (Sync.reconcileDivergence Sync.Fail incoming existingHash)
          "fail emits no op — the divergence stays surfaced for manual resolution"

        // KeepLocal: re-bind the SAME location to the EXISTING (local) hash, same kind, overriding
        match Sync.reconcileDivergence Sync.KeepLocal incoming existingHash with
        | Some(PT.PackageOp.SetName(loc, target)) ->
          Expect.equal loc location "the reconciling op targets the same location"
          let (PT.Hash h) = target.hash
          Expect.equal
            h
            existingHash
            "keep-local re-binds to the existing (local) hash, overriding the incoming"
          Expect.equal target.kind kind "the item kind is preserved"
        | other ->
          failtest $"keep-local should emit a SetName to the local hash, got {other}"

        // a non-SetName op never produces a reconciling op (only SetName can diverge)
        let nonSetName =
          PT.PackageOp.Undeprecate(
            PT.Reference.fromHashAndKind (PT.Hash incomingHash, kind)
          )
        Expect.isNone
          (Sync.reconcileDivergence Sync.KeepLocal nonSetName existingHash)
          "only SetName carries a name binding — other ops yield no reconciling op"
      }

      // Sync.reconcileBatch — the detect→resolve loop at batch level: find the incoming batch's
      // divergences (against the live `locations`) and emit the reconciling ops for a strategy.
      testTask
        "Sync.reconcileBatch detects a batch's divergences and emits reconciling ops per strategy" {
        // a real current location binding from the seed (same setup as the detectDivergences test)
        let! loc =
          Sql.query
            "SELECT owner, modules, name, item_hash, item_type, branch_id FROM locations WHERE unlisted_at IS NULL LIMIT 1"
          |> Sql.executeRowAsync (fun read ->
        ...
        let location : PT.PackageLocation =
          { owner = owner; modules = modules; name = name }
        let kind = PT.ItemKind.fromString itemType
        // incoming batch rebinds the location to a DIFFERENT hash → one divergence
        let incoming =
          PT.PackageOp.SetName(
            location,
            PT.Reference.fromHashAndKind (PT.Hash(existingHash + "_REMOTE"), kind)
          )

        // KeepLocal: the divergence yields one reconciling SetName back to the existing local hash
        let! keepLocalOps = Sync.reconcileBatch Sync.KeepLocal branchId [ incoming ]
        match keepLocalOps with
        | [ PT.PackageOp.SetName(rloc, rtarget) ] ->
          Expect.equal rloc location "the reconciling op re-binds the same location"
          let (PT.Hash h) = rtarget.hash
          Expect.equal
            h
            existingHash
            "back to the existing local hash (overriding the remote bind)"
        | other ->
          failtest $"expected one KeepLocal reconciling op, got {List.length other}"

        // LastWriterWins: no reconciling ops (the apply already bound to the remote = last writer)
        let! lww = Sync.reconcileBatch Sync.LastWriterWins branchId [ incoming ]
        Expect.isEmpty lww "last-writer-wins emits no reconciling ops"
      }

      // Sync.pullFromFile (rung 2 of local sync) — the one-call `dark sync pull <file>` op: read a
      ...
      testTask
        ...
        try
          // build the source file: a standalone op log carrying a copy of our (global) log
          execOn
            "CREATE TABLE package_ops (id TEXT PRIMARY KEY, op_blob BLOB NOT NULL, branch_id TEXT NOT NULL, commit_hash TEXT, origin_ts TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')))"
            []
          let! ops =
          ...
          Expect.equal
            cursor
            (int64 (List.length ops))
            "pull advanced the cursor to the source's last rowid (= its op count)"
      ...
      // Sync wire codec (rung 3 — HTTP transport): an op batch round-trips through encode → decode
      // byte-for-byte. The HTTP body reuses the existing op_blob bytes, so the wire carries exactly
      // what the rung-2 file transport reads; only the carrier differs.
      ...
      // WIP vs COMMITTED sync scope (a POLICY choice — see pr-sync-read-write.md): `opsSinceCommitted`
      // ships only committed ops (commit_hash set), excluding WIP — the mechanism for the multi-author
      // case (a coworker syncs your committed history, not your mid-edit WIP). The default `opsSince`
      // ships both (right for your own devices). Building block proven: committed ⊆ all, non-empty.
      testTask
      ...
      // the full HTTP payload path (rung 3): server encodes a batch → client decodes + applies it
      ...
      // (conflicts-and-resolutions.md). Stachu's case: an op authored EARLIER but applied LATER (it
      ...
      testTask
        ...
        Expect.equal
          afterEarly
          (Some(System.String('2', 64)))
          "an earlier-created op applied later does NOT override (Stachu's case)"
      }
