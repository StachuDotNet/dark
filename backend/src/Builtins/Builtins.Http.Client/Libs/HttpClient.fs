...
let fns (config : Configuration) : List<BuiltInFn> =
  ...
  [ { name = fn "httpClientRequest" 0
    ...
    //
    // Registered in the general builtin set, so any CLI-run Dark can reach it, including packages pulled
    // from a peer. `LibExecution.SyncTargets` is what keeps that from meaning "can read your loopback":
    // the guards are only off towards the relay you configured or a URL you typed.
    { name = fn "httpGetUnsafeBytes" 0
      ...
      fn =
        ...
        | _, _, _, [| DString uri |] ->
          uply {
            // The unguarded transport is for THIS instance's sync target and nothing else. Registered in
            // the general builtin set, it was reachable by any Dark the CLI runs, so a package pulled
            // from a peer could read the puller's own loopback. `SyncTargets` is seeded from the stored
            // relay and from URLs typed on the command line, neither of which pulled code can touch.
            if not (LibExecution.SyncTargets.isAllowed uri) then
              return
                Dval.resultError
                  KTBlob
                  KTString
                  (DString(LibExecution.SyncTargets.refusalMessage uri))
            else
    ...
    // only off towards origins `LibExecution.SyncTargets` knows about (the stored relay, or a URL typed on
