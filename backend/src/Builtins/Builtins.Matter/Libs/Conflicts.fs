...
let fns () : List<BuiltInFn> =
  [ { name = fn "conflictsList" 0
      ...
      returnType =
        TList(TTuple(TString, TString, [ TString; TString; TString; TString; TString; TString ]))
      description =
    ...
    { name = fn "conflictAcknowledge" 0
      typeParams = []
      parameters = [ Param.make "location" TString "the conflicted location (owner.modules.name)" ]
      returnType = TBool
      description =
      ...
      fn =
        ...
        | _, _, _, [ DString location ] ->
          uply {
            ...
            match
              conflicts
              |> List.tryFind (fun c -> c.location = location && c.status = "auto-resolved")
            with
            | Some c ->
    ...
    { name = fn "conflictKeep" 0
      typeParams = []
      parameters =
        [ Param.make "location" TString "the conflicted location (owner.modules.name)"
          Param.make "chosenHash" TString "the candidate content hash to keep (local or incoming)" ]
      returnType = TBool
      description =
