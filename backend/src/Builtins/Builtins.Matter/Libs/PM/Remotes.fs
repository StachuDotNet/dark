/// Builtins behind `dark remote` — the registered sync peers (a peer the tailnet daemon polls).
/// Thin wrappers over `LibDB.Remotes`. (Sync itself lands in a later PR; this is just the registry,
/// so you can wire peers ahead of turning sync on.)
