...
let fns () : List<BuiltInFn> =
  [ { name = fn "pmSyncPull" 0
    ...
    // ── HTTP transport (rung 3) — server read + client apply, base64 over the wire ──
    ...
    { name = fn "pmRemoteAdd" 0
      ...
      returnType = TUnit
      description =
        "Register (or update) a sync remote by name — a peer the tailnet-wide daemon will poll. `url`
         is the pollable target (an http(s) URL or a local data.db path). Idempotent by name."
      fn =
        (function
    ...
    { name = fn "pmRemoteList" 0
      ...
      returnType = TList TString
      description =
        "Registered sync remotes, one `name  →  url` line each (for `dark remote list`). Empty if
         none registered. (Cursor-only peers from a bare pull aren't here — register them to persist.)"
      fn =
        (function
        | _, _, _, [ DUnit ] ->
          uply {
    ...
    { name = fn "pmRemoteRemove" 0
      ...
      returnType = TBool
      description =
        "Unregister a sync remote by name. Returns true if it existed. (Its sync cursor, if any, is
         left intact — re-adding resumes where it left off.)"
      fn =
        (function
        | _, _, _, [ DString name ] ->
          uply {
