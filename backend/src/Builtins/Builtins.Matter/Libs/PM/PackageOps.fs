...
let private recordBranchEvent
  ...
  uply {
    ...
    // The op field is text: keeping it that way leaves every existing op's bytes, and therefore its
    // content hash, untouched.
    let op = PT.PackageOp.BranchEvent(string branchId, event, at)
  ...
  [ { name = fn "pmStabilizeHashes" 0
    ...
    { name = fn "scmSelectBranch" 0
      ...
      description =
        ...
        for name resolution and execution. Returns the branch now active."
      fn =
        (function
        | _, _, _, [| DUuid branchIdGuid |] ->
          uply {
