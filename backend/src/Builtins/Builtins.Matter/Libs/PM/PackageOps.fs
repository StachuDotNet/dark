...
/// The Add* subset of a batch: content, never names. What may fold into the shared content
/// tables when the batch lands on a branch; the name layer is what a branch keeps to itself.
let internal contentOpsOf (ops : List<PT.PackageOp>) : List<PT.PackageOp> =
  ops
  |> List.filter (fun op ->
    match op with
...
/// Fold a branch batch's content ops (Add*, never SetName) into the shared content tables, then
/// evaluate values so an expression-valued one has an `rt_dval` to eval. Needed so a branch item
/// resolves and carries its dependency edges; without it an authored or imported branch is
/// unusable.
let internal foldContentOps
  (exeState : ExecutionState)
  (ops : List<PT.PackageOp>)
  : Ply<unit> =
  uply {
    let contentOps = contentOpsOf ops
...
/// Refresh the process overlay after a write to <param branchId> -- but move it only when this
/// process is ON that branch: writing to another branch must not change what this caller resolves
/// against. Other branches are memoized, so forget them rather than leave a stale answer.
let internal refreshOverlayFor (branchId : PT.BranchId) : Ply<unit> =
  uply {
...
/// The `Result<Int, String>` constructors the count-returning scm builtins share.
let private resultOk = Dval.resultOk KTInt KTString
let private resultError = Dval.resultError KTInt KTString


/// A PackageOps builtin. Every one here is NotQueryable, needs no capabilities, takes no type
/// params and is not deprecated, so only what varies is spelled per declaration.
let private mk
  (name : string)
...
let fns (pm : PT.PackageManager) : List<BuiltInFn> =
  [ mk
      ...
      (TList(TCustomType(NR.ok (packageOpTypeName ()), [])))
    ...
    mk
      "pmUnresolvedNames"
      [ Param.make "ops" (TList(TCustomType(NR.ok (packageOpTypeName ()), []))) "" ]
      (TList(TTuple(TString, TList TString, [])))
    ...
    mk
      ...
      [ Param.make
        ...
        Param.make "ops" (TList(TCustomType(NR.ok (packageOpTypeName ()), []))) "" ]
      (TypeReference.result TInt TString)
           ...
           try
             ...
             if not branchId.IsMain then
               ...
               | false ->
                 ...
                 // Content folds into the shared content tables; the NAME layer is what a branch
                 // keeps to itself. Needed so an expression-valued branch value has an rt_dval to
                 // eval, and so propagation can see the branch item's dependency edges.
                 do! foldContentOps exeState stabilized
                 do! refreshOverlayFor branchId
    ...
    mk
      ...
      TUuid
      "The branch this process is on, as an id."
      Impure
      (function
       | _, _, _, [| DUnit |] ->
    ...
    mk
      "packageOpBindsAName"
      [ Param.make "blob" TBlob "a package_ops op_blob" ]
      TBool
    ...
    mk
      "packageOpFromBlobOption"
      [ Param.make "id" TUuid ""; Param.make "blob" TBlob "" ]
    ...
    mk
      "scmRecordBranchArchived"
      [ Param.make "branchId" TUuid "the branch that was archived" ]
      TUuid
         ...
         uply {
           ...
           try
             ...
             if List.length parsed <> List.length records then
               ...
               // Fold the content ops exactly as authoring onto a branch does -- see
               // scmAddOps: the overlay binds names, not bodies, so without this the
               // imported branch is unusable.
               do! foldContentOps exeState ops
               ...
               // An overlay this process is already holding predates the import, so drop it rather
               // than let a memoized read answer for the branch as it was before its ops arrived.
               do! refreshOverlayFor branchId
               return resultOk (Dval.int (bigint (int n)))
           with ex ->
             return resultError (Dval.string ex.Message)
