...
let rec private toJsonV0
  ...
  task {
    ...
    // nested types
    | DTuple(d1, d2, rest) ->
      do! w.writeArray (fun () -> Task.iterSequentially writeDval (d1 :: d2 :: rest))

    | DList(_, l) -> do! w.writeArray (fun () -> Task.iterSequentially writeDval l)
    ...
    | DEnum(_, _, _typeArgsDEnum, caseName, fields) ->
      do!
        w.writeObject (fun () ->
          ...
          w.writeArray (fun () ->
            fields |> Task.iterSequentially (fun fieldVal -> writeDval fieldVal)))
...
let parseJsonV0
  ...
  let rec convert (typ : TypeReference) (j : JsonElement) : Task<Dval> =
    ...
    | TBlob, JsonValueKind.Object ->
      ...
      DBlob(Persistent(hash, length)) |> Task.FromResult
    | TBlob, _ ->
      Exception.raiseInternal
