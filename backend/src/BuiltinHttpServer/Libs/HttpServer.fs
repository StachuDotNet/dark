  ...
  [ { name = fn "httpServerServe" 0
      ...
      fn =
        ...
        | exeState, _, _, [ DInt64 port; DApplicable handler ] ->
          // Outer fn returns DvalTask (= Ply<Dval> on this branch). The accept
          // loop itself is a Task<unit>, run-to-completion blockingly so the
          // builtin doesn't return until the listener is shut down.
          uply {
