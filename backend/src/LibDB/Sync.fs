/// The sync receiver/sender API (pr-sync-read-write) — the cohesive module the Dark HTTP
...
// ── Wire codec (rung 3 — HTTP/Tailscale transport) ─────────────────────────────────────────
...
// over the existing bytes — no new per-op serializer. Same bytes the file transport (rung 2)
...
/// (fail-closed) rather than misreading — the cheap op-format-stability guard the spec names, in the
...
/// The sender read (`GET /sync/events?since=cursor`): the ops the puller hasn't seen, as
/// `(rowid, id, origin_ts, op_blob)` — `op_blob` + the rowid cursor + the authoring stamp the
/// receiver needs for timestamp-LWW (carried adjacent, not inside the op).
let opsToSend (cursor : int64) : Task<List<int64 * System.Guid * string * byte[]>> =
  Inserts.opsSince cursor

/// The bootstrap read (`GET /sync/snapshot`): a new tailnet member reads the whole op log
/// once, then polls incrementally from the snapshot's cursor.
///
/// **Consistent by construction** — the cursor is the max rowid *among the ops just read*
/// (not a separate `SELECT MAX(rowid)` that could race a concurrent insert). So the (ops,
/// cursor) pair can never be torn: a later op gets a higher rowid and is picked up on the
/// peer's first incremental poll. Empty log → cursor 0. (This sidesteps the "snapshot must be
/// a consistent read" risk the spec flags, with no transaction/lock.)
let snapshot () : Task<List<int64 * System.Guid * string * byte[]> * int64> =
  task {
    let! ops = Inserts.opsSince 0L // the whole log, ascending rowid
    let cursor =
      match ops with
      | [] -> 0L
      | _ -> ops |> List.map (fun (rowid, _, _, _) -> rowid) |> List.max
    return (ops, cursor)
  }

/// Detect sync divergences in a remote batch BEFORE applying it (sync.md). For each incoming
...
/// How to auto-resolve a sync divergence (sync.md / conflict-dispatch). `detectDivergences`
/// surfaces the divergence as data — "divergence is data we fix later"; this is the *fix* half,
/// the op-emitting counterpart to the `ConflictDispatch` policy. A strategy maps a divergent
/// incoming `SetName` to a reconciling op (or none):
///   - `LastWriterWins`: the incoming `SetName` already applied = the last writer won → NO extra
///     op (the apply itself is the resolution).
///   - `KeepLocal`: re-bind the location to the EXISTING (local) hash, overriding the incoming.
///   - `Fail`: emit nothing — leave the divergence surfaced for manual resolution.
type ResolutionStrategy =
  | LastWriterWins
  | KeepLocal
  | Fail

/// The reconciling op for a divergent incoming `SetName` under a strategy, or None when no op is
/// needed (`LastWriterWins` — already applied) or wanted (`Fail` — left surfaced). `existingHash`
/// is the local binding the incoming diverges from (as `detectDivergences` reports it). A
/// `ConflictDispatch` policy resolving a `CSyncDivergence` to "keep local" yields the op this
/// builds; the receiver applies it after the batch to re-assert the local binding.
let reconcileDivergence
  (strategy : ResolutionStrategy)
  (incoming : PT.PackageOp)
  (existingHash : string)
  : Option<PT.PackageOp> =
  match incoming with
  | PT.PackageOp.SetName(loc, target) ->
    match strategy with
    | LastWriterWins -> None // the incoming SetName IS the last write; nothing to add
    | Fail -> None // leave the divergence surfaced; no auto-op
    | KeepLocal ->
      // re-bind the location to the existing (local) hash, same item kind, overriding incoming
      let existingRef =
        PT.Reference.fromHashAndKind (PT.Hash existingHash, target.kind)
      Some(PT.PackageOp.SetName(loc, existingRef))
  | _ -> None // only SetName carries a name binding / can diverge


/// The detection→resolution loop at batch level: detect the incoming batch's divergences and
/// produce the reconciling ops for a strategy. Returns the ops (the caller decides whether/when to
/// apply them — the receiver path stays non-blocking by default; auto-applying these is opt-in).
/// Empty for `LastWriterWins`/`Fail` (the apply itself resolves / the divergence stays surfaced).
let reconcileBatch
  (strategy : ResolutionStrategy)
  (branchId : PT.BranchId)
  (ops : List<PT.PackageOp>)
  ...
  task {
    ...
    return
      pairs
      |> List.choose (fun (op, existingHash) ->
        reconcileDivergence strategy op existingHash)
  }
...
/// Rung 2 of local sync: PULL another instance's ops into THIS instance by reading the peer's
...
/// `dark sync pull <other-data.db>`; rung 3 (HTTP) only swaps the source connection for a wire
...
let pull
  ...
  task {
    ...
    cmd.CommandText <-
      "SELECT rowid, id, op_blob, branch_id, commit_hash, origin_ts FROM package_ops "
      + "WHERE rowid > $cursor ORDER BY rowid ASC"
    ...
    for (branchId, commitHash) in batches do
      ...
      // data; never blocks the apply (the keystone's "divergence is data we fix later").
...
/// The client apply-half of the HTTP transport (rung 3): decode a wire batch (the body of a
