...
let createInMemory (ops : List<PT.PackageOp>) : PT.PackageManager =
  ...
  for op in ops do
    ...
    // None of these change what a name points at -- a Decide records what a person decided ABOUT a
...
/// so it answers empty without a query. That is why this takes a plain `BranchId`: `None` and
/// `Some main` used to mean the same thing, which is a sentinel wearing an Option.
