...
/// A regenerable projection in the per-branch cache: which `package_ops` kinds *fold* it
/// (a full rebuild replays these) and which kinds *dirty* it (an incremental update refolds
/// only projections an incoming op invalidates). Op-kind names match the `PackageOp` DU cases.
/// This is the projection **registry** — the small new abstraction the ops⊥projections PR names.
type Projection =
  { table : string
    foldsOpKinds : Set<string>
    dirtiedBy : Set<string> }

/// The 6 regenerable projections — every table the op-fold writes. `deprecations` IS one:
/// it's folded from `Deprecate`/`Undeprecate` ops (its `annotation_blob` reconstructs from the
/// op), so it's regenerable and `Seed.export` strips it too (Stachu's call — it had been
/// preserved, an inconsistency, now fixed). NOT `package_blobs` (canonical content — op-playback
/// never writes it) nor the op log / branch / commit / account state.
let projectionRegistry : List<Projection> =
  [ { table = "package_functions"
      foldsOpKinds = Set.ofList [ "AddFn" ]
      dirtiedBy = Set.ofList [ "AddFn" ] }
    { table = "package_types"
      foldsOpKinds = Set.ofList [ "AddType" ]
      dirtiedBy = Set.ofList [ "AddType" ] }
    { table = "package_values"
      foldsOpKinds = Set.ofList [ "AddValue" ]
      dirtiedBy = Set.ofList [ "AddValue" ] }
    { table = "locations"
      foldsOpKinds = Set.ofList [ "SetName" ]
      dirtiedBy = Set.ofList [ "SetName"; "RevertPropagation" ] }
    { table = "package_dependencies"
      foldsOpKinds = Set.ofList [ "AddFn"; "AddType"; "AddValue" ]
      dirtiedBy = Set.ofList [ "AddFn"; "AddType"; "AddValue" ] }
    { table = "deprecations"
      foldsOpKinds = Set.ofList [ "Deprecate"; "Undeprecate" ]
      dirtiedBy = Set.ofList [ "Deprecate"; "Undeprecate" ] } ]

/// The tables a full rebuild clears + refolds — derived from the registry (single source of truth).
let projectionTables : List<string> = projectionRegistry |> List.map (fun p -> p.table)
...
let rebuildDirtied (opKinds : Set<string>) : Task<int64> =
  task {
    ...
    let groups =
      relevant |> List.groupBy (fun (_, b, c) -> (b, c)) |> Map.toList
    for ((branchId, commitHash), g) in groups do
      do! PackageOpPlayback.applyOps branchId commitHash (g |> List.map (fun (op, _, _) -> op))
