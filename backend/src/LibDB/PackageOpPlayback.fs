...
let private applySetName
  ...
  task {
    ...
    // ── timestamp-LWW (conflicts-and-resolutions.md): order this binding by the op's CREATION time,
    ...
    let! thisTs =
      task {
        ...
        cmd.Parameters.AddWithValue("$id", string thisOpId) |> ignore<SqliteParameter>
    ...
    let! curBinding =
      task {
        ...
        cmd.Parameters.AddWithValue("$item_type", itemTypeStr) |> ignore<SqliteParameter>
        cmd.Parameters.AddWithValue("$branch_id", string branchId) |> ignore<SqliteParameter>
        use! reader = cmd.ExecuteReaderAsync()
        let! hasRow = reader.ReadAsync()
        if hasRow then
