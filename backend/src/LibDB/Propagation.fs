...
let propagate
  ...
  task {
    ...
    // Main is an id like any other here. This used to send "" for main, which matched no row: main's
    // policy rows are stored under main's id, and the inheritance clause needs a real id to compare
    // against. A pin made on main was silently ignored by the cascade, on main AND on branches.
