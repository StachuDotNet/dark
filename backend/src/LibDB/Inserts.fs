...
let nextOriginTs () : string =
  lock originTsLock (fun () ->
    ...
    let next = if nowMs > lastOriginTs then nowMs else lastOriginTs.AddMilliseconds 1.0
    lastOriginTs <- next
    next.ToString("yyyy-MM-ddTHH:mm:ss.fffZ", System.Globalization.CultureInfo.InvariantCulture))
