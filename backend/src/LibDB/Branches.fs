...
///
/// THIS RULE EXISTS TWICE: `PackageOpPlayback.applyDecide` folds the same op for MAIN, under
...
///
/// `Resolve` counts, same as in `SCM.PackageOps.bindingFromOp`: a resolution IS a rebind that also
/// carries a decision id. Counting only `SetName` leaves a name bound solely by resolving a
