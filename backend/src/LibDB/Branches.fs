...
/// The branch's OWN latest binding for one name: what its frontier says that name points at.
///
/// Ordered by the ops' stamps, so "latest" is the branch's own last word rather than whatever the
/// query happened to return first.
let private ownBindingFor
  (ops : List<System.Guid * PT.PackageOp>)
  (owner : string, modules : string, name : string)
  : Option<PT.PackageLocation * PT.Reference> =
  ops
  |> List.choose (fun (_, op) ->
    match op with
    | PT.PackageOp.SetName(loc, target, _) when
      loc.owner = owner
      && String.concat "." loc.modules = modules
      && loc.name = name
      ->
...
/// RESOLVE keep-mine: author an OVERRIDE that re-states the branch's binding with a stamp that wins
/// LWW, then set the base to the parent's current.
///
...
///
/// The stamp is still MAX(origin_ts) + 1s rather than "now", because authoring stamps can run ahead of
/// wall clock (see `LibDB.OriginTs`) and a peer's op can carry a future stamp. That is safe to do to an
/// op nobody has seen yet, and unsafe to do to one they have.
let resolveKeepMine
  (branchId : PT.BranchId)
  (fqn : string)
  : Task<Result<unit, string>> =
  task {
    ...
    | Some key ->
      let! ops = ownFrontierOps branchId
      match ownBindingFor ops key with
      | None ->
        // The name: this reaches a person through `dark resolve`, and they typed a name.
        let! branchName = nameForId branchId
        return
      ...
      | Some(location, target) ->
        let! stamp =
          Sql.query
            "SELECT strftime('%Y-%m-%dT%H:%M:%fZ', MAX(origin_ts), '+1 second') AS ts
             FROM package_ops"
          |> Sql.executeRowAsync (fun read -> read.string "ts")

        // Idempotence is the caller's, per `Decision`: keyed on the name AND the stamp, so resolving
        // twice records two decisions rather than dedupping into one that then loses to what came
        // between them.
        ...
        let! _ = storeDeltaOpsStamped branchId [ (op, stamp) ]
        do! setBaseToParentFor branchId key
        return Ok()
  }
