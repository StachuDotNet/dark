...
let fns () : List<BuiltInFn> =
  [ { name = fn "int8Mod" 0
    ...
    { name = fn "int8Remainder" 0
      ...
      fn =
        let resultOk r = Dval.resultOk KTInt8 KTString r |> Task.FromResult
        (function
    ...
    { name = fn "int8Add" 0
      ...
      fn =
        ...
        | _, vm, _, [ DInt8 a; DInt8 b ] ->
          try
            DInt8(Checked.(+) a b) |> Task.FromResult
        ...
        | _, vm, _, [ DInt8 number; DInt8 exp ] ->
          (try
            if exp < 0y then
              ...
              (bigint number) ** (int exp) |> int8 |> DInt8 |> Task.FromResult
           with :? System.OverflowException ->
             RTE.Ints.OutOfRange |> RTE.Int |> raiseRTE vm.threadID)
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
        ...
        | _, vm, _, [ DInt8 a; DInt8 b ] ->
          if b = int8 0 then
            ...
            if result < -128 || result > 127 then
              ...
              Task.FromResult(DInt8(int8 result))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8GreaterThan" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a; DInt8 b ] -> Task.FromResult(DBool(a > b))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8ToString" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a ] -> Task.FromResult(DString(string a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8ToFloat" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a ] -> Task.FromResult(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8Random" 0
      ...
      fn =
        ...
        | _, _, _, [ DInt8 a; DInt8 b ] ->
          ...
          int8Result |> DInt8 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "int8Sqrt" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a ] -> Task.FromResult(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8Parse" 0
      ...
      fn =
        ...
        | _, _, _, [ DString s ] ->
          try
            s |> System.SByte.Parse |> DInt8 |> resultOk |> Task.FromResult
          with
    ...
    { name = fn "int8FromUInt8" 0
      ...
      fn =
        ...
        | _, _, _, [ DUInt8 a ] ->
          if a > 127uy then
            Dval.optionNone KTInt8 |> Task.FromResult
          else
            Dval.optionSome KTInt8 (DInt8(int8 a)) |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8BitwiseAnd" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a; DInt8 b ] -> Task.FromResult(DInt8(a &&& b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8BitwiseNot" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a ] -> Task.FromResult(DInt8(~~~a))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "int8ShiftLeft" 0
      ...
      fn =
        (function
        | _, _, _, [ DInt8 a; DInt8 b ] -> Task.FromResult(DInt8(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
