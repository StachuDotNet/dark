...
module ParseError =
  ...
  let toDT (e : ParseError) : Dval =
    ...
    let typeName = FQTypeName.fqPackage (PackageRefs.Type.Stdlib.uint8ParseError ())
...
let fns () : List<BuiltInFn> =
  [ { name = fn "uint8Mod" 0
    ...
    { name = fn "uint8Add" 0
      ...
      fn =
        ...
        | _, vm, _, [ DUInt8 a; DUInt8 b ] ->
          try
            DUInt8(Checked.(+) a b) |> Task.FromResult
          with :? System.OverflowException ->
            RTE.Ints.OutOfRange |> RTE.Int |> raiseRTE vm.threadID |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8Power" 0
      ...
      fn =
        ...
        | _, vm, _, [ DUInt8 number; DUInt8 exp ] ->
          (try
            (bigint number) ** (int exp) |> uint8 |> DUInt8 |> Task.FromResult
           with :? System.OverflowException ->
             RTE.Ints.OutOfRange
             |> RTE.Int
             |> raiseRTE vm.threadID
        ...
        | _, vm, _, [ DUInt8 a; DUInt8 b ] ->
          if b = 0uy then
            ...
            if result < 0 || result > 255 then
              ...
              Task.FromResult(DUInt8(uint8 result))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8GreaterThan" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a; DUInt8 b ] -> Task.FromResult(DBool(a > b))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8ToString" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a ] -> Task.FromResult(DString(string a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8ToFloat" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a ] -> Task.FromResult(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8Random" 0
      ...
      fn =
        ...
        | _, _, _, [ DUInt8 a; DUInt8 b ] ->
          ...
          uint8Result |> DUInt8 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "uint8Sqrt" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a ] -> Task.FromResult(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8Parse" 0
      ...
      returnType =
        let errorType =
          FQTypeName.fqPackage (PackageRefs.Type.Stdlib.uint8ParseError ())
        TypeReference.result TUInt8 (TCustomType(NR.ok errorType, []))
      ...
      fn =
        ...
        | _, _, _, [ DString s ] ->
          try
            s |> System.Byte.Parse |> DUInt8 |> resultOk |> Task.FromResult
          with
    ...
    { name = fn "uint8FromInt8" 0
      ...
      fn =
        ...
        | _, _, _, [ DInt8 a ] ->
          if a < 0y then
            Dval.optionNone KTUInt8 |> Task.FromResult
          else
            Dval.optionSome KTUInt8 (DUInt8(uint8 a)) |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8BitwiseAnd" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a; DUInt8 b ] -> Task.FromResult(DUInt8(a &&& b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8BitwiseNot" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a ] -> Task.FromResult(DUInt8(~~~a))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint8ShiftLeft" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a; DUInt8 b ] -> Task.FromResult(DUInt8(a <<< int b))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
