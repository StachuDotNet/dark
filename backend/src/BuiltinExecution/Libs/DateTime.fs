  ...
  [ { name = fn "dateTimeParse" 0
    ...
    { name = fn "dateTimeToStringISO8601BasicDate" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          (DarkDateTime.toDateTimeUtc d).ToString("yyyyMMdd") |> DString |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeNow" 0
      ...
      fn =
        (function
        | _, _, _, [ DUnit ] ->
          Instant.now () |> DarkDateTime.fromInstant |> DDateTime |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Impure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeToday" 0
      ...
      fn =
        ...
        | _, _, _, [ DUnit ] ->
          ...
          Task.FromResult(DDateTime(DarkDateTime.T(now.Year, now.Month, now.Day, 0, 0, 0)))
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Impure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeAddSeconds" 0
      ...
      fn =
        ...
        | _, _, _, [ DDateTime d; DInt64 s ] ->
          d + (NodaTime.Period.FromSeconds s) |> DDateTime |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = SqlBinOp "+"
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeGreaterThan" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d1; DDateTime d2 ] -> Task.FromResult(DBool(d1 > d2))
        | _ -> incorrectArgs ())
      sqlSpec = SqlBinOp ">"
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeToSeconds" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          (DarkDateTime.toInstant d).ToUnixTimeSeconds() |> DInt64 |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeYear" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> d.Year |> int64 |> Dval.int64 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeMonth" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> d.Month |> int64 |> Dval.int64 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeDay" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> d.Day |> int64 |> Dval.int64 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeWeekday" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> d.DayOfWeek |> int64 |> DInt64 |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeHour" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> Task.FromResult(Dval.int64 d.Hour)
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeMinute" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> Task.FromResult(Dval.int64 d.Minute)
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeSecond" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] -> Task.FromResult(Dval.int64 d.Second)
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeAtStartOfDay" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          DarkDateTime.T(d.Year, d.Month, d.Day, 0, 0, 0) |> DDateTime |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeSubtract" 0
      ...
      fn =
        ...
        | _, _, _, [ DDateTime endDate; DDateTime startDate ] ->
          ...
          diff.TotalSeconds |> System.Math.Round |> int64 |> DInt64 |> Task.FromResult
        | _ -> incorrectArgs ())
