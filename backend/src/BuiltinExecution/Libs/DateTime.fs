  ...
  [ { name = fn "dateTimeParse" 0
    ...
    { name = fn "dateTimeToStringISO8601BasicDate" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          (DarkDateTime.toDateTimeUtc d).ToString("yyyyMMdd")
          |> DString
          |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
    ...
    { name = fn "dateTimeToday" 0
      ...
      fn =
        ...
        | _, _, _, [ DUnit ] ->
          ...
          Task.FromResult(
            DDateTime(DarkDateTime.T(now.Year, now.Month, now.Day, 0, 0, 0))
          )
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Impure
    ...
    { name = fn "dateTimeYear" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          d.Year |> int64 |> Dval.int64 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeMonth" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          d.Month |> int64 |> Dval.int64 |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeWeekday" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          d.DayOfWeek |> int64 |> DInt64 |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeAtStartOfDay" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          DarkDateTime.T(d.Year, d.Month, d.Day, 0, 0, 0)
          |> DDateTime
          |> Task.FromResult
        | _ -> incorrectArgs ())
    ...
    { name = fn "dateTimeToMilliseconds" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          (DarkDateTime.toInstant d).ToUnixTimeMilliseconds()
          |> DInt64
          |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
    ...
    { name = fn "dateTimeAddMilliseconds" 0
      ...
      fn =
        ...
        | _, _, _, [ DDateTime d; DInt64 ms ] ->
          d + (NodaTime.Period.FromMilliseconds ms) |> DDateTime |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "dateTimeMillisecond" 0
      ...
      fn =
        (function
        | _, _, _, [ DDateTime d ] ->
          d.Millisecond |> int64 |> Dval.int64 |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotQueryable
      previewable = Pure
      deprecated = NotDeprecated }
