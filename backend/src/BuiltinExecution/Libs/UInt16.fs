  ...
  [ { name = fn "uint16Mod" 0
      ...
      fn =
        ...
        | _, vm, _, [ DUInt16 v; DUInt16 m ] ->
          if m = 0us then
            RTE.Ints.ZeroModulus |> RTE.Int |> raiseRTE vm.threadID |> Task.FromResult
          else
            let result = v % m
            let result = if result < 0us then m + result else result
            Task.FromResult(DUInt16(result))
        | _ -> incorrectArgs ())
    ...
    { name = fn "uint16Power" 0
      ...
      fn =
        ...
        | _, vm, _, [ DUInt16 number; DUInt16 exp ] ->
          (try
            (bigint number) ** (int exp) |> uint16 |> DUInt16 |> Task.FromResult
           with :? System.OverflowException ->
             RTE.Ints.OutOfRange |> RTE.Int |> raiseRTE vm.threadID |> Task.FromResult)
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
    ...
    { name = fn "uint16Divide" 0
      ...
      fn =
        ...
        | _, vm, _, [ DUInt16 a; DUInt16 b ] ->
          if b = 0us then
            RTE.Ints.DivideByZeroError |> RTE.Int |> raiseRTE vm.threadID |> Task.FromResult
          else
            let result = a / b
            if
            ...
            then
              ...
              Task.FromResult(DUInt16(uint16 result))
    ...
    { name = fn "uint16GreaterThan" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt16 a; DUInt16 b ] -> Task.FromResult(DBool(a > b))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint16ToString" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt16 a ] -> Task.FromResult(DString(string a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint16ToFloat" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt16 a ] -> Task.FromResult(DFloat(float a))
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint16Random" 0
      ...
      fn =
        ...
        | _, _, _, [ DUInt16 a; DUInt16 b ] ->
          ...
          Task.FromResult(DUInt16(uint16Result))
    ...
    { name = fn "uint16Sqrt" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt16 a ] -> Task.FromResult(DFloat(sqrt (float a)))
        | _ -> incorrectArgs ())
    ...
    { name = fn "uint16Parse" 0
      ...
      fn =
        ...
        | _, _, _, [ DString s ] ->
          try
            s |> System.Convert.ToUInt16 |> DUInt16 |> resultOk |> Task.FromResult
          with
    ...
    { name = fn "uint16FromInt8" 0
      ...
      fn =
        ...
        | _, _, _, [ DInt8 a ] ->
          if (a < 0y) then
            Dval.optionNone KTUInt16 |> Task.FromResult
          else
            Dval.optionSome KTUInt16 (DUInt16(uint16 a)) |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
    ...
    { name = fn "uint16FromUInt8" 0
      ...
      fn =
        (function
        | _, _, _, [ DUInt8 a ] -> DUInt16(uint16 a) |> Task.FromResult
        | _ -> incorrectArgs ())
      sqlSpec = NotYetImplemented
      previewable = Pure
      deprecated = NotDeprecated }
