...
module Hashing =
  ...
  let private normalizeExpr (e : PT.Expr) : PT.Expr = norm (ref 0) Map.empty e

  /// Alpha-normalize a value: its body's binders become canonical.
  let private normalizeValue (v : PT.PackageValue.PackageValue) : PT.PackageValue.PackageValue =
  ...
  let private normalizeFn (f : PT.PackageFn.PackageFn) : PT.PackageFn.PackageFn =
