...
- **Telemetry already streaming.** CLI events land in one greppable JSONL stream, with confirmed event names across `cli.*` (e.g. `cli.total`), `commandExec`, `httpserver.*` (`listening`/`request`/`serve`/`shutdown`), and `seed.*` (`apply`/`db`/`evaluate`/`generate`) — all verified. (A `test.suite.*` stream is assumed by the harness design but was *not* found in the current tree; confirm or add it before the harness depends on it.) The harness leans on this directly.
