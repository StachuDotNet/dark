...
`design/distributed-event-sourcing.md`). State is what you get by
...
`design/structural-editor.md`). Code is just a value (an AST is a
...
Async work parks on the event bus (see `design/event-bus.md` and
`design/async.md`): materializations awaiting an LLM, evals
