...
- **Location-transparency is the shared win.** If the substrate routes by `appId`, a remote `appId` is the same code path as local — exactly BEAM's superpower, and exactly what the wire protocol wants (see [sync.md](../stable-and-syncing/sync.md)).
