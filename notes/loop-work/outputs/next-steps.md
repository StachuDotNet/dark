...
detail lives in the [design/](design/) docs; this says **what to build first and in
...
carries only ops and commits.** See [design/apps-surface.md](design/apps-surface.md)
for the end-state and [design/sync.md](design/sync.md) for the substrate.

## Priority order

**1. Stable & Syncing (do this).** Everything below points at the two-builds-syncing
target. **2. Removing `.dark` files — punted** (see
[design/bootstrap.md](design/bootstrap.md); blocked on stable env + working sync).
**3. PDD — resting** ([design/pdd.md](design/pdd.md); spike, not advancing). The
...
1. **Tailscale transport + ping/pong.** A `Builtins.Tailscale` package
   ...
   end-to-end. ([design/sync.md](design/sync.md), [design/remote-access.md](design/remote-access.md))
2. **Conflict-dispatch skeleton.** `Conflict` + `Resolution` sum types and a
   ...
   runtime errors) routes through it later. ([design/conflicts.md](design/conflicts.md))
3. **Event-bus + scheduler core.** The thin F# `EventBus` (publish / subscribe /
   ...
   hot-reload, and async all rest on. ([design/event-bus.md](design/event-bus.md),
   [design/async.md](design/async.md))
...
7. **`print-md` as an App + the `dark apps` surface.** Declare print-md as an App,
   ...
   ([design/apps-surface.md](design/apps-surface.md),
   [design/distributed-event-sourcing.md](design/distributed-event-sourcing.md))
...
- **Multi-user / `matter.darklang.com` / public funnel** — after self-sync works;
  approval-as-ops is designed ([design/sync.md](design/sync.md)) but deferred.
- **Interactive capability grants** — deliberately deferred
  ([design/capabilities.md](design/capabilities.md)); grants are instance-specific for now.
