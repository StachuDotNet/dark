...
- **Conflict-blind vs. conflict-carrying op-playback** — does the generic playback
  ...
  ([distributed-event-sourcing.md](design/distributed-event-sourcing.md))
- **Does WIP sync, and how does it fold?** The recurring one (appears in three docs).
  ...
  ([conflicts.md](design/conflicts.md), [sync.md](design/sync.md), [distributed-event-sourcing.md](design/distributed-event-sourcing.md))
- **Async core before or under the event bus** — replace Ply with our own scheduler
  ...
  interface, not Ply.* ([async.md](design/async.md), [event-bus.md](design/event-bus.md))
- **Default sync target** — auto-sync to the server on install, or opt-in? *Lean:
  opt-in, local-first (mirror git `remote add`).* ([sync.md](design/sync.md))
- **Sync granularity** — *lean: per-branch with namespace as a filter.* ([sync.md](design/sync.md))
- **Inbound-conflict dispatch timing** — resolve arriving conflicts immediately, or
  batch and surface on next CLI interaction? *Per-session policy.* ([sync.md](design/sync.md), [conflicts.md](design/conflicts.md))
...
- **Parse-time uniformity** — defer-to-runtime (placeholder + run-time conflict) vs.
  an explicit `ParsePolicy`. ([conflicts.md](design/conflicts.md))
...
- **Backpressure** (producer outpaces consumers), **cancellation** (timeout vs.
  ...
  frame that never wakes). ([event-bus.md](design/event-bus.md))
...
- **Auto-parallelization** — default-on vs. flag. ([async.md](design/async.md))
...
- **Apps-runtime coupling** to the daemon; **idle shutdown** policy; **protocol
  ...
  ([cli-daemon.md](design/cli-daemon.md))

## Capabilities

- **HttpClient restriction grammar** — the detailed spec is said to live in a vault
  note. **BLOCKED: location not found** (see [vault-organization.md](vault-organization.md)).
...
- **FileSystem** — ships as a boolean; structured per-directory/read-write scoping is
  future. ([capabilities.md](design/capabilities.md))
...
- **Versioning + update**; **discovery** (public namespace / registry / shared ref).
  ([apps-surface.md](design/apps-surface.md))
...
- **Render-target abstraction** (`View` node set; F# adapter vs Dark renderer);
  **runner placement** (how much F# vs Dark); **`Stdlib.UI` content**; **real-time
  ...
  ([composable-mvu.md](design/composable-mvu.md))
- **Editor**: op granularity vs. typing latency; where the tiny model runs;
  holes-vs-partial-text; the parser/`compile` dependency. ([structural-editor.md](design/structural-editor.md))
...
- **`dark on` syntax**; **stream encoding** (*lean: chunked HTTP*); **pair-share device
  ...
  ([remote-access.md](design/remote-access.md))
...
- **Break surfacing** (cause vs. effect site — *lean: both, linked*); **blast-radius
  ...
  body, apply new on next call*). ([hot-reload.md](design/hot-reload.md))
...
- **Identity at scale** (autonomous agents); **multi-tenant boundaries**; **adversarial
  ...
  ([cohabitation.md](design/cohabitation.md), [identity.md](design/identity.md))
...
off-limits to the loop (recommendation in [vault-organization.md](vault-organization.md)).
