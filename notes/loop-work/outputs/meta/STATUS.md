...
- **Pass 27 (genuine deepening — worked example):** Added `example-app.md`: a shared
...
- **Pass 10 (iteration) — FULL-TREE REVIEW COMPLETE / saturation reached:** Read and verified the
  ...
  reached the diminishing-returns point its own `where-the-loop-struggles.md`
...
- **Pass 5 (iteration):** Verified `sync.md` thorough. Fixed a real cross-doc inconsistency:
  ...
  frontmatter — realigned to the `notes/pdd/projects/` shape (goal + acceptance-criteria +
...
- **Pass 3 (iteration):** Critical re-review of the Stable & Syncing cluster against feedback.md —
  ...
  `apps-surface.md` (the `dark apps` install/fork/run surface, anchored on the print-md
...
- `vault/current-experiment/Design/Dark Async Plan.md` — coworker's doc, review-only, kept as
  reference (informed `async.md`; not edited).
