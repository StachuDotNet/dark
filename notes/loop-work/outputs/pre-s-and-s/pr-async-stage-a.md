...
| **Every `Builtins.*` assembly** (all 9: Pure, Http.Client, Http.Server, Random, Time, Cli, CliHost, Language, Matter) | Each builtin definition adds `effects = …`. **This is the bulk** — a few hundred call sites. Mechanical but the *correctness-critical* part: a mis-declared `Pure` that actually does IO is a soundness bug (next section). |
