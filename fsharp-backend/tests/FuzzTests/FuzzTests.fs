...
let knownBad config =
  [ OCamlInterop.tests // fails in so many unique ways
    ...
    Json.LibJwtJson.tests // fails on DObj (map [("𣏕", DNull); ("", DNull)])
...
let tests config =
  [ testList "knownGood" (knownGood config)
    testList "knownBad" (knownBad config)
    testList "playground" (Playground.tests config) ]
...
let main args =
  ...
  let config: FuzzTestConfig =
    ...
    match args |> List.ofArray |> List.pairwise |> List.tryFind(fun (l, r) -> l = "--fscheck-max-tests") with
