...
let urlEncodeExcept (keep : string) (s : string) : string =
  ...
  let encodeByte (b : byte) : byte array =
    ...
    // cut/copy this fn to its usages; Prelude to only have idiomatic version
...
let gid () : uint64 =
  try
    ...
    // This currently keeps us at 30 bits - we'd like to instead use 64, or settle on 63
    // Current blocker is client (needs research)
